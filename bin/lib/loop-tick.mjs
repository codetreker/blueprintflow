...
export function cmdCompleteTick(args) {
  ...
  if (!unit) {
    console.error("Usage: bf-harness complete-tick --unit <id> --artifacts <comma-sep> --description <text> --dir <path>");
    process.exit(1);
  }
  ...
  if (state._written_by !== WRITER_SIG || !state._write_nonce) {
    errors.push("state was not written by bf-harness — possible direct edit detected");
  }
...
function _writeCheckpoint(dir, ctx, warnings) {
  ...
  const md = [
    ...
    "",
    `> Auto-generated by bf-harness complete-tick — ${new Date().toISOString()}`,
    "",
    `## Current`,
    `- **Unit**: ${unit} (${unitType})`,
    `- **Status**: ${status}`,
