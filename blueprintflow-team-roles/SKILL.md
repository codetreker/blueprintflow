...
- 文档 PR (Architect/PM/QA/Designer/Security) 全部用临时 clone
...
- **Ping/Pong 沉默检测**: 派活给 persistent agent (Dev/Architect/PM/QA等) 后, 如 30min 内无 idle_notification 也无 PR push 也无任何 SendMessage 回报, 启动 ping 协议:
 ...
 **How to apply**: 30min 沉默是阈值不是死规, 看任务复杂度调 (e.g. e2e 调试 60min 沉默正常, schema migration 30min 沉默异常). 用户拍板可豁免 (e.g. "DevA 在 debug 不要打扰").
