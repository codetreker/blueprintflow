...
- Dev → 下个 milestone 实施 (临时 clone 不冲突主 worktree)
- Architect → 下一波 spec brief / 蓝图 patch / 老 PR review
- PM → 下一波立场反查 / 文案锁 / demo 截屏
- QA → 下一波 acceptance template / REG 翻牌 / e2e flake fix
...
### 4. cron 输出格式
- 一句话报当前推进 (PR # + 1 句目标)
- Hard blocker (PR fail 超时 / review 长时间未响应) 单独列详情
...
| Spec / 4 件套 | 4 件套全员都 commit 进 worktree (Architect spec / QA acceptance / PM stance + content-lock), 不是单独一份 |
...
**Dev (dev)**: 当前 milestone 拆段 N+1 / 上 PR 暴露 bug 救火 / 下一 milestone schema spike
**Architect (architect)**: review queue / 下一 spec brief / 老蓝图 patch
**PM**: 立场反查表 / demo 截屏文案 / README/onboarding 文案锁
**QA**: acceptance template / REG 翻牌 / e2e flake fix
