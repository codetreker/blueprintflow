...
- 关键约束：State 字段只能取 Draft、Ready、Tasking、Completed；Capability 字段只能填一个（执行能力）；Acceptance Criteria 每条带 `{id}|{capability}` marker（验收能力，跟执行能力区分开）；`## Evidence` section 必须存在；Evidence 每条用 `{evidence-id}|{ac-id}|{kind}: requirement` 绑定到本 task 已存在的 AC；每条 task AC 至少有一条 Evidence；同 task 内 evidence id 唯一；kind 只能是 `command`、`file`、`artifact`、`review-note`、`screenshot`；requirement 不能为空。
