...
- **`core_type` / `accepts` / `produces` unvalidated by harness** — Stage 3 probe `brainstorm-task` (commit 655be3a) confirmed `bf-harness` silently accepts these BF additions on flow JSON without validation. Stage 4 needs a Pack-aware validator that enforces shape and enum membership.
- **Branching/back edges not rendered by `bf-harness viz`** — Stage 3 probes `brainstorm-task` (655be3a) and `close-leaf-task` (c26c67b) flagged the happy-path renderer only draws forward PASS edges; ITERATE/FAIL back-edges (and convergence onto a single node like `implement`) are in the JSON but invisible. Stage 4 viz should render branching edges so reviewers see the full graph.
- **Node-spawns-child-runs not supported by harness** — Stage 3 probe `loop-milestone` (commit 64dad9a) flagged `bf-harness` has no notion of nested runs; `loop.dispatch-children` describes the target behavior. Stage 4 `bf-run` must implement child-run dispatch (e.g. `bf-run execute --parent-run <id> <child-wo>`, nesting under `<parent-run-dir>/children/<child-id>/`).
- **Edge values are scalar verdicts, not structured payloads** — Stage 3 probe `loop-milestone` (commit 64dad9a) flagged that `aggregate.FAIL` can only route to one downstream node, forcing re-dispatch of the entire ready-set rather than the failed subset. Cleaner Stage 4 alternative: edge values become `{verdict, scope}` objects so the dispatcher can target the affected subset. Documented as a Core flow-edge schema limitation; workaround today is idempotent dispatch in the build agent.
- **Loop-type-aware default budgets** — Stage 3 probe `loop-milestone` (commit 64dad9a) flagged that static `maxLoopsPerEdge` / `maxTotalSteps` caps don't scale with child count discovered at runtime; wide milestones (>10 children) may trip 5/30 caps. Stage 4 may want loop-type-aware default budgets sized at `dispatch-children` entry.
- **`mkdir` ownership for child WO creation unspecified** — Stage 3 probe `breakdown-milestone-to-task` (commit 6433a89) flagged that core-contracts §180/§363 phrase child WO creation as flow-scoped without a `bf-harness create-child-wo` subcommand. Stage 4 should either add a harness command to formalize child WO creation or explicitly document agent-side filesystem ownership here.
...
- **`rolesDir` / `protocolDir` `../` escapes rejected** — Stage 3 probes `brainstorm-task` (655be3a), `breakdown-milestone-to-task` (6433a89), `loop-milestone` (64dad9a), and `close-leaf-task` (c26c67b) all omitted these keys because the current harness rejects `../` escapes when resolving them, and Pack flows need to share Core roles across `roles/` (Core) and `packs/<pack>/roles/` (Pack). Stage 4 dispatcher work should add a multi-root role resolver (flow rolesDir → Pack roles → Core roles per §"Role resolution") so these keys can be re-introduced.
- **`milestone.json` schema gap** — Stage 3 probe `breakdown-milestone-to-task` (commit 6433a89) found `packs/product-engineering/schemas/` only contained `task.json` despite `routing` referencing `milestone`. A minimal `milestone.json` was authored inline (states `new → shaped → broken_down → children_done → done`). Revisit during Stage 6 schema hardening; the Pack's declared `schemas` set must be complete before Stage 4 dispatcher routes by schema.
- **`Teamlead` persona has no Pack role file (by design, needs documenting)** — Stage 3 probe `close-leaf-task` (commit c26c67b) flagged that v6 `bf-team-roles` ships a 7-role roster but Teamlead maps to the BF orchestrator itself (not a dispatchable subagent). Stage 6 should add a pack-level note (e.g. in `packs/product-engineering/README.md`) recording that Teamlead = orchestrator and is not a role file.
- **v6 role names vs OPC vendor role names** — Stage 3 probe `close-leaf-task` (commit c26c67b) chose OPC vendor names (`engineer`, `tester`, `pm`, `architect`, `designer`, `security`) over v6 names (`Dev`, `QA`, `PM`, `Architect`, `Designer`, `Security`). Stage 4 or 6 should add documentation aliases in `packs/product-engineering/reference-v6/README.md`.
- **PR open/merge orchestration is out of flow** — Stage 3 probe `close-leaf-task` (commit c26c67b) scoped the close flow to in-WO state transitions only; worktree/branch/PR lifecycle is dispatcher concern. Stage 4 should bind a post-PASS hook (equivalent of `bf-git-workflow`) that opens/merges the PR using gate handshake metadata (task id, branch name, evidence pointers).
