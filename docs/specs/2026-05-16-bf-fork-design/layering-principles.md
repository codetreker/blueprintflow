...
**Pack 不带运行时**。运行时是 bf 包根(`bin/`、`SKILL.md`)的事;Pack 只是告诉 bf-run 和 bf-harness:"在本领域,按这套形状跑就对了"。
...
## 3. Pack 是 npm 独立包还是 bf 包子目录

### 决定:**v1 内嵌**,设计上为外置预留口子

```
@codetreker/bf  (npm 包根 = bf-skill 根)
├── SKILL.md
├── bin/
├── pipeline/
├── roles/                          ← Core roles
...
bf 包发到 npm。Pack 内嵌在 `packs/<id>/`。

未来若有第三方 Pack 或第一方 Pack 需要独立发版:bf-run 的 Pack 发现机制加一行扫描 `@codetreker/bf-pack-*` npm sibling 包(或本地 `node_modules/@codetreker/bf-pack-*`),架构无需重构。

### 候选方案与对比

| 方案 | 形态 | 适用阶段 |
|---|---|---|
| A. 每 Pack 独立 npm 包,bf 不内嵌任何 Pack | `@codetreker/bf` + `@codetreker/bf-pack-product-engineering` 平级 | 多 Pack 阶段、社区贡献活跃后 |
| B. 全部内嵌 bf 包 | `packs/*` | **v1 阶段(当前)** |
| C. 混合:第一方内嵌,第三方独立 | `packs/*` + `@codetreker/bf-pack-*` | 长期形态;v1 实现 B 部分,留发现机制扩展位 |

### 选 B 的理由

1. **v1 只有 1 个 Pack**。拆独立 npm 包现阶段只是给 bf-run 加跨包扫描和版本依赖的复杂度。
2. **product-engineering Pack 与 BF Core 短期同节奏迭代**。Stage 3 的迁移探针就是用 Pack 反过来压测 Core,Pack 改一行 Core 也可能改一行;锁同一 npm 包更顺手。
...
| 层 | 例子 | 位置 |
|---|---|---|
| Core roles | planner / architect / tester / security / a11y / compliance / devil-advocate / skeptic-owner / user-simulator | `roles/` |
| Pack roles | pm / designer / frontend / backend / devops / mobile / new-user / active-user / churned-user | `packs/<pack-id>/roles/` |
