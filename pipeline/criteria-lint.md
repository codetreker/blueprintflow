...
**Every mode** — both YOLO (`/bf <task>`) and interactive (`/bf <task> -i`).

The lint runs after the orchestrator drafts `acceptance-criteria.md` and BEFORE `bf-harness init`. Init is gated — if criteria-lint fails, init refuses to start.

```
1. User invokes /bf <task>
2. Orchestrator analyzes task, selects tier
3. Orchestrator drafts $SESSION_DIR/acceptance-criteria.md
4. bf-harness criteria-lint $SESSION_DIR/acceptance-criteria.md  <-- THIS
5. If PASS: bf-harness init --flow X --tier Y
...
```bash
$ bf-harness criteria-lint $SESSION_DIR/acceptance-criteria.md
...
⛔ DoD failed criteria-lint after 3 revision attempts:
  ❌ {remaining failures}
Use /bf edit-criteria to fix manually, or /bf force-init to bypass.
```

`/bf force-init` sets `flow-state._criteria_lint_bypassed = true`, which is surfaced as a warning in downstream gates.
...
- Do NOT use `/bf force-init` in YOLO mode without surfacing to user first
