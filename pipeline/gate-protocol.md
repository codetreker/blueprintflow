...
```bash
bf-harness synthesize $SESSION_DIR --node {UPSTREAM_NODE_ID}
```
...
```bash
bf-harness route --node {GATE_ID} --verdict {VERDICT} --flow {FLOW_TEMPLATE}
```
...
```bash
bf-harness transition --from {GATE_ID} --to {NEXT_NODE} --verdict {VERDICT} --flow {FLOW_TEMPLATE} --dir $SESSION_DIR
```
...
- `allowed = false` → cycle limit reached. Surface to user with escape options:
  - `/bf pass` — force PASS, advance to the PASS edge target
  - `/bf stop` — terminate flow, preserve state
  - `/bf goto <node>` — manual override (still checked against cycle limits)
...
- **Done:** `🎉 Flow complete.`
- **Blocked:** `⛔ Cycle limit reached at {gate}. Use /bf pass, /bf stop, or /bf goto <node>.`

## Anti-Patterns

- ❌ Overriding the synthesized verdict with your own judgment
- ❌ Determining the next node by reading SKILL.md tables — use `bf-harness route`
