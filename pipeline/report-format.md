...
```bash
mkdir -p ~/.bf/reports
bf-harness report . --mode {mode} --task "{task}" --challenged N --dismissed M --downgraded K > ~/.bf/reports/{filename}.json
```
...
Browse past reports with `/bf replay`. See `../replay.md` for the full replay skill.
