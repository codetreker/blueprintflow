...
PLAN
$HARNESS init-loop --skip-scope --dir .bf --plan .bf/plan.md >/dev/null 2>/dev/null
NONCE=$(python3 -c "import json; d=json.load(open('.bf/loop-state.json')); print(d.get('_write_nonce','MISSING'))")
...
"
OUT=$($HARNESS complete-tick --dir .bf --unit F1.1 --status completed --artifacts dummy.txt 2>/dev/null)
assert_output_contains "detects bad writer" "$OUT" "not written by bf-harness"

echo ""
echo "--- 2.3: Reject wrong unit ---"
setup_loop
OUT=$($HARNESS complete-tick --dir .bf --unit F1.2 --status completed --artifacts dummy.txt 2>/dev/null)
assert_output_contains "explains expected unit" "$OUT" "expected unit"

echo ""
echo "--- 2.4: Reject modified plan ---"
setup_loop
echo "# tampered" >> .bf/plan.md
OUT=$($HARNESS complete-tick --dir .bf --unit F1.1 --status completed --artifacts dummy.txt 2>/dev/null)
...
setup_loop
OUT=$($HARNESS complete-tick --dir .bf --unit F1.1 --status blocked --description "waiting for API key" 2>/dev/null)
assert_field_eq "accepts blocked with description" "$OUT" "completed" "true"

echo ""
echo "--- 2.6: Reject blocked without description ---"
setup_loop
OUT=$($HARNESS complete-tick --dir .bf --unit F1.1 --status blocked 2>/dev/null)
...
OUT=$($HARNESS complete-tick --dir .bf --unit F1.1 --status completed --artifacts test-result.json 2>/dev/null)
...
OUT=$($HARNESS complete-tick --dir .bf --unit F1.1 --status completed --artifacts test-result2.json 2>/dev/null)
...
OUT=$($HARNESS complete-tick --dir .bf --unit F1.1 --status completed --artifacts bad-artifact.json 2>/dev/null)
