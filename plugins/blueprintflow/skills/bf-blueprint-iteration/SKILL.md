...
| Tasks | `docs/tasks/` | The execution path from next to current: Phase -> Milestone plan, milestone readiness review, task execution, milestone progress |
...
- Blueprint lock does not require a complete task split. It locks the Phase/Milestone plan, dependency boundaries, and acceptance direction.
- `bf-milestone-breakdown` reviews milestone readiness before concrete task work starts.
- Readiness review does not mean task execution has started; task-level state lives in `docs/tasks/README.md`, `milestone.md`, and task folders after execution creates them.
...
After the user names a concrete iteration objective, read `references/lifecycle.md` for the full flow: scan backlog → write/resume `docs/blueprint/next/` with a status ledger → lock anchors → plan `docs/tasks` as Phase -> Milestone with dependency boundaries → run `bf-milestone-breakdown` for milestone readiness review → run `bf-task-execute` one task per PR → run `bf-milestone-progress` after accepted tasks → promote accepted scope into `docs/blueprint/current/`. Use [references/promotion-checklist.md](references/promotion-checklist.md) for accepted-scope promotion. Reminder period is project-defined in `AGENTS.md`.
...
# docs/blueprint/next/ anchors lock → plan docs/tasks/ Phase -> Milestone + dependency boundary
# selected milestone → bf-milestone-breakdown reviews execution readiness
