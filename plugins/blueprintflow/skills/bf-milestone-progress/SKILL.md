...
description: "Part of the Blueprintflow methodology. Use when a task is accepted, the next task handoff must be decided, a milestone may close, or Phase exit readiness must be checked."
---
...
- `milestone.md` needs the next task handoff decided.
...
| Next task handoff or blocker | `milestone.md`, `docs/tasks/README.md` |
...
6. Decide whether the Milestone needs another concrete task, can close, or is blocked. Use accepted-task evidence, remaining Milestone acceptance, real blockers, and any task rows already created by execution.
7. If another task is needed, keep the next ledger `Work` as `IMPLEMENTING` with `Milestone path` still pointed at the milestone folder, record the reason in `milestone.md`, and hand off to `bf-task-execute` to create or resume the concrete task from current context.
...
- The next task handoff names why another concrete task is needed, or why the milestone can close.
...
- Assuming a next task already exists when `bf-task-execute` must create it from current context.
- Selecting parallel tasks without checking real dependency/conflict evidence.
