...
- Dev A coordinates work in the Teamlead-assigned task worktree
- Other Dev coordinators use whichever Teamlead-assigned `.worktrees/<task>` scope they are given
...
# Working directory
Dev: <repo-root>/.worktrees/<task> (created by the Teamlead)
...
# Default work queue
- Implementation of the next sub-section (N+1) of the current task
- Firefighting bugs surfaced by the previous PR (P0)
- Schema spike for the next task
