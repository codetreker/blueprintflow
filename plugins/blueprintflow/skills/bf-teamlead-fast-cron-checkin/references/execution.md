...
- Dev -> implement an active task already created or confirmed by `bf-task-execute`
- Architect -> review queue / blueprint patches / spec for an active task
- PM -> stance cross-checks / content lock / demo screenshots for an active task
- QA -> acceptance evidence / REG flips / e2e flake fixes for an active task
...
- c) **forward**: the next planned milestone or active task from `docs/tasks` (readiness / `bf-task-execute` / spec / acceptance / implementation / stance)
...
3. If `Work` is `PENDING` and the selected milestone lacks readiness review, dispatch `bf-milestone-breakdown`.
4. If `Work` is `IMPLEMENTING`, read `docs/tasks/README.md`, `milestone.md`, and the relevant task `progress.md` when one exists, then dispatch the next missing action: readiness fix/review, `bf-task-execute`, implementation owner, PR gate owner, acceptance owner, `bf-milestone-progress`, or `bf-phase-exit-gate`.
...
7. Dispatch the exact role action needed in the milestone plan, readiness review, task folder, or task worktree.
...
- Readiness review has no review or evidence movement past the threshold.
- Readiness passed but no concrete task has started by the next fast check-in.
...
**Dev**: implement an active task created or confirmed by `bf-task-execute` / firefighting bugs exposed by the last PR.
**Architect**: review queue / active task spec brief / patches to accepted current or locked next blueprints.
...
- **A selected milestone with no readiness review** — run `bf-milestone-breakdown`.
- **A readiness review with no review or evidence movement past the stuck threshold** — dispatch the missing reviewer, blocker owner, or evidence recorder.
- **A readiness-passed milestone with no concrete task started** — route to `bf-task-execute` to create or resume the task.
