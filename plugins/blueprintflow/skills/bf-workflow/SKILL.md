...
3. Lock next scope and record gate evidence: bf-blueprint-iteration -> bf-phase-plan (Phase -> Milestone + first task seed)
4. Break down the selected milestone with fresh gate evidence: bf-milestone-breakdown (reviewed task skeletons)
5. Start task loop:
...
| Lock selected next anchors and record Next lock integrity evidence | `bf-blueprint-iteration` |
| Split locked next-blueprint anchors with fresh gate evidence into execution Phases/Milestones, with task seed | `bf-phase-plan` |
| Break a selected milestone with fresh gate evidence into reviewed task skeletons | `bf-milestone-breakdown` |
...
Route backward when prerequisites are missing: if stances are unsettled, use `bf-brainstorm`; if next product shape is not locked, use `bf-blueprint-write`; if Next lock integrity evidence is missing, stale, or failed, STOP and return to `bf-blueprint-iteration` to produce valid lock evidence before planning; if Phase/Milestone planning is missing after fresh gate evidence, use `bf-phase-plan`; if milestone task skeletons are missing or unreviewed after fresh gate evidence, use `bf-milestone-breakdown`; if task state is inconsistent, use `bf-task-state-standard`; if a concrete task has not started, use `bf-task-execute` before implementation or PR review.
...
- Next-blueprint anchors lock and pass the Next lock integrity gate before Phase/Milestone planning or milestone breakdown; `bf-milestone-breakdown` creates reviewed task skeletons before task work; accepted work promotes to current only after coding and acceptance pass.
