...
3. Plan locked next scope: bf-blueprint-iteration -> bf-phase-plan (Phase -> Milestone + dependency boundary)
4. Review the selected milestone: bf-milestone-breakdown (execution readiness)
5. Start task loop:
...
| Split locked next-blueprint anchors into execution Phases/Milestones with dependency boundaries | `bf-phase-plan` |
| Review a selected milestone for execution readiness | `bf-milestone-breakdown` |
...
Route backward when prerequisites are missing: if stances are unsettled, use `bf-brainstorm`; if next product shape is not locked for execution planning, use `bf-blueprint-write` or `bf-blueprint-iteration`; if Phase/Milestone planning is missing, use `bf-phase-plan`; if milestone readiness is missing or unreviewed, use `bf-milestone-breakdown`; if task state is inconsistent, use `bf-task-state-standard`; if a concrete task has not started, use `bf-task-execute` before implementation or PR review.
...
- `docs/tasks/` is the next -> current execution path: Phase -> Milestone first, then milestone readiness review, then concrete task work.
...
- Next-blueprint anchors lock before Phase/Milestone planning; `bf-milestone-breakdown` reviews milestone readiness before task work; accepted work promotes to current only after coding and acceptance pass.
