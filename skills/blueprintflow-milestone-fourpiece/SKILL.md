...
**Git workflow**: Teamlead creates `.worktrees/<milestone-or-issue>` + branch. 4-piece authors stack commits in the same worktree. Teamlead is the sole PR opener. See `blueprintflow-git-workflow`.
...
After 4 pieces, before code, Dev writes `design.md`. Four-role review (Architect / PM / Security / QA) must all ✅ before coding starts. Same worktree, same PR. Full spec in `blueprintflow-implementation-design`.
...
```
follow skill blueprintflow-milestone-fourpiece
```
