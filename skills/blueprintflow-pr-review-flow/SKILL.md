...
description: "PR open 到 merged 的标准流程: 双 review 路径 (Architect 架构 + QA acceptance + 涉敏感加 Security) + Security checklist (12 类 lazy reference) + 三联签 merge gate (CI + LGTM + 任务完成度) + 标准 squash, 永远不 admin/ruleset bypass. 触发: PR open 后派 review / 收 LGTM 后 merge / merge 之前 CI fail 处理. 反触发: PR 还没 open (走 milestone-fourpiece + implementation-design) / 实施还没完 (走 4 件套 / design review) / draft PR 还没 ready / 蓝图层规则审查 (走 brainstorm)."
version: 1.0.0
---

# PR Review Flow

PR open 之后到 merged 的标准流程.

## PR 出来之前: implementation design 4 ✅ 必备

milestone PR 开之前, 涉及代码的 milestone **必须**已经过 implementation design 4 角色 review:

- 设计文档: `docs/implementation/design/<milestone>.md` (Dev 主写)
- 4 角色 ✅: Architect (架构 + 规则) / PM (用户价值 + UX) / Security (鉴权 / 数据隔离 / cross-org) / QA (可测性 + 边界 case)
- review 走 worktree 内通讯 / PR comment, 不开独立 PR
- 任一 ❌ 就阻塞 — 不允许开 milestone PR

完整规格见 `blueprintflow-implementation-design`.

非代码 milestone (docs-only / config-only / 字面调整) 可以跳这一步, 直接进 PR review.
...
- LGTM 评论必须引具体清单条目 (比如 "§1 鉴权 ✅, §8 IDOR 见 line 42 已防"), 反"笼统过审"

详见 `references/security-checklist.md` (12 类, 每类 bullet + "为什么 + 怎么验").

## 🚫 永久禁 (硬红线 — 不可商量)

下面这些手段**永远禁用**, 任何场景任何理由都不允许. 这是用户 2026-04-29 拍板的硬红线, 不接受 "临时" / "兜底" / "flaky" / "急" 任何借口:
...
**为什么是硬红线**:
- admin bypass 会盖住 bug — flaky 后面是真 bug 的概率比表面看高
...
- ❌ 派 "admin merge agent" / "batch admin merge agent" — agent 名字本身已经弃用
...
- 同一个 case 在不同 PR 随机 fail → flaky 信号
...
|---|---|---|---|
| Dev 实施 PR | Architect (架构) | QA (acceptance) | — |
| Architect spec brief PR | Dev (实施视角) | QA (acceptance 可机器化) | PM (规则) |
| PM stance / content-lock PR | Architect (架构) | QA (acceptance) | — |
| QA acceptance template / 状态翻 PR | Architect (架构) | PM (规则, 仅 v0 规则相关时) | — |
...
review 内容必须包含锚点 (跟 spec / stance / acceptance 字面 cross-check):
- 跟 #<other-PR> 字面对得上吗?
- §X.Y 的边界守住了吗?
- 跟字面一致的模板 (比如跨 milestone 共享结构体模板) 一致吗?

**Merge 三联签** (CI + LGTM + 任务完成度):
- ① CI 真过 (statusCheckRollup 全 SUCCESS, 永远不 admin/ruleset bypass)
- ② ≥1 个非 author 的 LGTM (gh pr review --approve 或 LGTM 评论 from 不同 reviewer 身份)
- ③ **Teamlead 审 PR body 的 Acceptance + Test plan 全勾上** (`gh pr view <N> --json body | jq -r .body | grep -cE "^- \[ \]"` 必须 == 0)

三联签全过 → 标准 squash merge. 任一缺 → 不合.

详细的 merge gate 协议见 `blueprintflow-teamlead-fast-cron-checkin §5`. 任务完成度判据 (一个 milestone 一个 PR 协议下) 在那里展开.
...
- ❌ subagent prompt 不带具体 cross-ref PR # / commit SHA (review 失去字面验证能力)
...
2. 检查 ≥1 个非 author 的 LGTM (gh pr review --approve 或 LGTM 评论 from 不同 agent role)
3. 如果 PR template lint 缺字段:
   patch body via gh api -X PATCH /repos/<owner>/<repo>/pulls/<N> --input <(jq ...)
   close+reopen 触发 lint rerun (修 body, **不**修 lint enforcement)
4. CI 真过 (statusCheckRollup 全 SUCCESS) + mergeable=CLEAN + ≥1 个非 author LGTM
...
5. 任何 fail 场景退给 author 修, 不 bypass:
   ...
   - PR template lint regex 误报 → 修 lint regex 让真合规的 body 过, 不 bypass
   - DIRTY → author rebase main
   - 真 flaky → 重 trigger CI 重跑, 还 fail 就退给 author 修根因
6. 报 merge 时间 + SHA. 报告里**禁止**出现 "admin" / "ruleset disable" / "bypass" 任何字眼
```

注: `gh pr edit --body` 在某些环境不生效, 用 `gh api PATCH` 直接 patch JSON.

#> Batch merge 模式详见 `references/batch-merge.md`.

## 跨 review 例子: 规则漂移抓出来

> **实战案例 (Borgee):** QA review acceptance 时自查, 发现字段名跟 Architect spec brief 对不上 (字段改名没同步), 当场 patch 修齐.

这就是双轨 review 起作用 — spec 写 A 形态, acceptance 自然按 A 写, drift 就能发现.
