...
setup
seed_mode_a_success "$BASE/projects/p/clean-wo"
STDOUT=$(node --input-type=module -e "
  import('$REPO_ROOT/bin/lib/cmd-accept.mjs').then(async (m) => {
...
grep -q "^State: Accepted" "$BASE/projects/p/clean-wo/bf.md" || fail "bf.md state not flipped"
grep -q "^State: Ready" "$BASE/projects/p/clean-wo/task-a/spec.md" || fail "task-a state not flipped"
grep -q "^State: Ready" "$BASE/projects/p/clean-wo/task-b/spec.md" || fail "task-b state not flipped"
grep -q "^Updated: 2026-05-19 12:34" "$BASE/projects/p/clean-wo/bf.md" || fail "bf.md Updated not set"
cleanup
...
grep -q "^State: Draft" "$BASE/projects/p/clean-wo/bf.md" || fail "bf.md unexpectedly modified"
cleanup
...
setup
seed_mode_a_success "$BASE/projects/p/clean-wo"
sed -i.bak 's/^Pack: engineering/Pack: nope/' "$BASE/projects/p/clean-wo/bf.md"
STDOUT=$(node --input-type=module -e "
  import('$REPO_ROOT/bin/lib/cmd-accept.mjs').then(async (m) => {
    process.stdout.write(JSON.stringify(await m.cmdAccept({
      baseHome: '$BASE', projectSlug: 'p', woId: 'clean-wo', repoRoot: '$REPO',
