...
write_verify_result() {
  ...
  local dir="$BASE/works/wo-1/runs/reviews/round_1"
  mkdir -p "$dir"
...
- AC-1
EOF
  touch -d "2026-06-11 10:00:00" "$BASE/works/wo-1/bf.md"
  touch -d "2026-06-11 10:00:00" "$BASE/works/wo-1/task-a/spec.md"
  touch -d "2026-06-11 10:00:00" "$BASE/works/wo-1/task-b/spec.md"
...
setup
sed -i.bak 's/^State: Implementing/State: Accepted/' "$BASE/works/wo-1/bf.md"
write_success_verify
run_complete_json
...
setup
sed -i.bak 's/^- \[x\] AC-1/- [ ] AC-1/' "$BASE/works/wo-1/bf.md"
write_success_verify
run_complete_json
assert_json_field "$STDOUT" .ok false
...
setup
sed -i.bak 's/^State: Completed/State: Tasking/' "$BASE/works/wo-1/task-b/spec.md"
write_success_verify
run_complete_json
assert_json_field "$STDOUT" .ok false
...
setup
write_success_verify
touch -d "2026-06-11 10:01:00" "$BASE/works/wo-1/bf.md"
run_complete_json
assert_json_field "$STDOUT" .ok false
...
grep -q "^State: Completed" "$BASE/works/wo-1/bf.md" || fail "bf.md not Completed"
cleanup
