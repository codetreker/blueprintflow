...
BASE=$(make_temp_home)
mkdir -p "$BASE/works/wo-x"
echo "hi" > "$BASE/works/wo-x/bf.md"
...
[ ! -d "$BASE/works/wo-x" ] || fail "wo-x not removed"
...
BASE=$(make_temp_home)
mkdir -p "$BASE/works/wo-y"
echo "hi" > "$BASE/works/wo-y/bf.md"
export BF_HOME="$BASE"
...
[ ! -d "$BASE/works/wo-y" ] || fail "wo-y not removed"
