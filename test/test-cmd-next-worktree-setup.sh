...
# If a later managed-Git task in the batch fails setup, earlier Ready tasks
# must not leave unrecorded branches or worktrees behind.
make_git_repo_with_origin
prepare_wo true
sed -i.bak 's/^- task-b: task-a/- task-b/' "$BASE/works/wo-1/bf.md"
sed -i.bak 's/^Requires-Worktree: .*/Requires-Worktree: true/' "$BASE/works/wo-1/task-b/spec.md"
...
EXPECTED_WORKTREE_A="$PRIMARY/.worktrees/works/wo-1/task-a"
EXPECTED_WORKTREE_B="$PRIMARY/.worktrees/works/wo-1/task-b"
mkdir -p "$EXPECTED_WORKTREE_B"
printf "not a git worktree\n" > "$EXPECTED_WORKTREE_B/README.txt"
STDOUT=$(cmd_next_json "$PRIMARY")
assert_json_field "$STDOUT" .ok false
assert_match "$STDOUT" "worktree" "batch managed setup failure message"
grep -q "^State: Ready" "$BASE/works/wo-1/task-a/spec.md" || fail "batch managed setup failure mutated task-a"
grep -q "^State: Ready" "$BASE/works/wo-1/task-b/spec.md" || fail "batch managed setup failure mutated task-b"
grep -q "^State: Accepted" "$BASE/works/wo-1/bf.md" || fail "batch managed setup failure mutated bf"
grep -q "^Branch:$" "$BASE/works/wo-1/task-a/spec.md" || fail "batch managed setup failure recorded task-a Branch"
grep -q "^Worktree:$" "$BASE/works/wo-1/task-a/spec.md" || fail "batch managed setup failure recorded task-a Worktree"
git -C "$PRIMARY" show-ref --verify "refs/heads/$EXPECTED_BRANCH_A" >/dev/null 2>&1 && fail "batch managed setup failure left task-a branch"
[ ! -e "$EXPECTED_WORKTREE_A" ] || fail "batch managed setup failure left task-a worktree"
rm -rf "$ROOT"

# If Ready worktree setup fails while an existing independent task is already
# Tasking, next still returns the Tasking task for coordinator resume and leaves
# the Ready task unclaimed.
BASE=$(make_temp_home)
copy_fixture clean-wo "$BASE/works/wo-1"
sed -i.bak 's/^Id: clean-wo/Id: wo-1/' "$BASE/works/wo-1/bf.md"
...
assert_json_field "$STDOUT" .tasks.length 1
assert_json_field "$STDOUT" .tasks.0.taskId task-a
grep -q "^State: Tasking" "$BASE/works/wo-1/task-a/spec.md" || fail "Tasking resume task was mutated"
grep -q "^State: Ready" "$BASE/works/wo-1/task-b/spec.md" || fail "failed Ready task was claimed"
grep -q "^Branch:$" "$BASE/works/wo-1/task-b/spec.md" || fail "failed Ready task recorded Branch"
grep -q "^Worktree:$" "$BASE/works/wo-1/task-b/spec.md" || fail "failed Ready task recorded Worktree"
rm -rf "$BASE"

# If Git state changes after batch preflight and a later prepare fails, earlier
# successfully-created worktrees are rolled back instead of left unrecorded.
make_git_repo_with_origin
prepare_wo true
sed -i.bak 's/^- task-b: task-a/- task-b/' "$BASE/works/wo-1/bf.md"
sed -i.bak 's/^Requires-Worktree: .*/Requires-Worktree: true/' "$BASE/works/wo-1/task-b/spec.md"
...
STDOUT=$(cmd_next_json "$PRIMARY")
assert_json_field "$STDOUT" .ok false
assert_match "$STDOUT" "worktree" "post-preflight race failure message"
grep -q "^State: Ready" "$BASE/works/wo-1/task-a/spec.md" || fail "post-preflight failure mutated task-a"
grep -q "^State: Ready" "$BASE/works/wo-1/task-b/spec.md" || fail "post-preflight failure mutated task-b"
grep -q "^State: Accepted" "$BASE/works/wo-1/bf.md" || fail "post-preflight failure mutated bf"
grep -q "^Branch:$" "$BASE/works/wo-1/task-a/spec.md" || fail "post-preflight failure recorded task-a Branch"
grep -q "^Worktree:$" "$BASE/works/wo-1/task-a/spec.md" || fail "post-preflight failure recorded task-a Worktree"
git -C "$PRIMARY" show-ref --verify "refs/heads/$EXPECTED_BRANCH_A" >/dev/null 2>&1 && fail "post-preflight failure left task-a branch"
git -C "$PRIMARY" show-ref --verify "refs/heads/$EXPECTED_BRANCH_B" >/dev/null 2>&1 || fail "post-preflight failure removed unproven task-b branch"
[ ! -e "$EXPECTED_WORKTREE_A" ] || fail "post-preflight failure left task-a worktree"
[ -e "$EXPECTED_WORKTREE_B/README.txt" ] || fail "post-preflight failure removed user path conflict"
rm -rf "$ROOT"
