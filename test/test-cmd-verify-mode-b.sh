...
setup() {
  ...
  BASE=$(make_temp_home)
  mkdir -p "$BASE"
  copy_fixture "$fixture" "$BASE/works/wo-1"
  ...
  [ -f "$BASE/works/wo-1/task-b/spec.md" ] && sed -i.bak 's/^State: Draft/State: Ready/' "$BASE/works/wo-1/task-b/spec.md"
...
grep -qE "^- \[x\] AC-1\|" "$BASE/works/wo-1/task-a/spec.md" || fail "AC-1 not flipped"
grep -q "^State: Tasking" "$BASE/works/wo-1/task-a/spec.md" || fail "verify should leave task-a Tasking"
grep -q "^State: Implementing" "$BASE/works/wo-1/bf.md" || fail "bf.md state changed unexpectedly"
...
setup clean-wo
write_blocker_review "$BASE/works/wo-1/task-a/runs/reviews/round_1" tester 1
run_verify_b
assert_json_field "$STDOUT" .status "FAIL"
grep -qE "^- \[ \] AC-1\|" "$BASE/works/wo-1/task-a/spec.md" || fail "AC-1 unexpectedly flipped"
grep -q "^State: Tasking" "$BASE/works/wo-1/task-a/spec.md" || fail "task-a state changed on Blocker"
cleanup

# Case 3: FAIL on missing signoff (no AC signed)
setup clean-wo
write_signed_review "$BASE/works/wo-1/task-a/runs/reviews/round_1" tester 1 ""
run_verify_b
assert_json_field "$STDOUT" .status "FAIL"
RESULT_FILE="$BASE/works/wo-1/task-a/runs/reviews/round_1/verify-result.md"
...
run_verify_b
assert_json_field "$STDOUT" .status "SUCCESS"
grep -qE "^- \[x\] AC-1\|" "$BASE/works/wo-1/task-a/spec.md" || fail "OR semantics: AC-1 should flip"
RESULT_FILE="$BASE/works/wo-1/task-a/runs/reviews/round_1/verify-result.md"
...
setup clean-wo
sed -i.bak 's/^State: Implementing/State: Draft/' "$BASE/works/wo-1/bf.md"
run_verify_b
