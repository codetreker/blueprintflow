...
setup() {
  ...
  for t in task-a task-b; do
    ...
    sed -i.bak 's/^Updated: .*/Updated: 2026-05-19 10:00/' "$BASE/wo-1/$t/spec.md"
  done
...
setup
ROUND_DIR="$BASE/wo-1/runs/reviews/round_1"
...
grep -q "^State: Implementing" "$BASE/wo-1/bf.md" || fail "bf.md state changed on FAIL"
cleanup
...
grep -q "^State: Implementing" "$BASE/wo-1/bf.md" || fail "bf.md state changed on stale FAIL"
cleanup
...
rm -f "$BASE/wo-1/task-b/spec.md.bak"
ROUND_DIR="$BASE/wo-1/runs/reviews/round_1"
write_signed_review "$ROUND_DIR" tester 1 "AC-1"
touch -d "2026-05-19 11:00" "$ROUND_DIR/result_tester_1.md"
run_verify_c
