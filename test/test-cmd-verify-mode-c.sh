...
grep -qE "^- \[x\] AC-1\|" "$BASE/works/wo-1/bf.md" || fail "bf.md AC-1 not flipped"
grep -q "^State: Implementing" "$BASE/works/wo-1/bf.md" || fail "bf.md should remain Implementing after verify"
cleanup
...
grep -q "^State: Implementing" "$BASE/works/wo-1/bf.md" || fail "bf.md state changed on FAIL"
cleanup
...
grep -q "^State: Implementing" "$BASE/works/wo-1/bf.md" || fail "bf.md state changed on stale FAIL"
cleanup
...
rm -f "$BASE/works/wo-1/task-b/spec.md.bak"
ROUND_DIR="$BASE/works/wo-1/runs/reviews/round_1"
write_signed_review "$ROUND_DIR" tester 1 "AC-1"
touch -d "2026-05-19 11:00" "$ROUND_DIR/result_tester_1.md"
run_verify_c
