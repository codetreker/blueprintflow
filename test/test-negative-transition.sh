...
TRANS=$($HARNESS transition --from code-review --to test-design --verdict PASS --flow build-verify --dir .bf 2>/dev/null || true)
...
TRANS2=$($HARNESS transition --from build --to gate --verdict PASS --flow build-verify --dir .bf 2>/dev/null || true)
...
rm -rf .bf
$HARNESS init --flow build-verify --entry build --dir .bf 2>/dev/null >/dev/null
TRANS3=$($HARNESS transition --from build --to code-review --verdict PASS --flow build-verify --dir .bf 2>/dev/null || true)
...
EOF
touch .bf/nodes/build/x
TRANS4=$($HARNESS transition --from build --to code-review --verdict FAIL --flow build-verify --dir .bf 2>/dev/null || true)
