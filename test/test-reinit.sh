...
setup_stalled_loop
OUT=$($HARNESS reinit-loop --skip-scope --dir .bf --unit F1.1 --sub-units "F1.1a: implement-api — Build API layer, F1.1b: implement-ui — Build UI layer, F1.1c: review-fullstack — Review both layers" 2>/dev/null)
...
STATE_STATUS=$(python3 -c "import json; d=json.load(open('.bf/loop-state.json')); print(d['status'])")
STATE_NEXT=$(python3 -c "import json; d=json.load(open('.bf/loop-state.json')); print(d['next_unit'])")
...
PLAN
$HARNESS init-loop --skip-scope --dir .bf --plan .bf/plan.md >/dev/null 2>/dev/null
OUT=$($HARNESS reinit-loop --skip-scope --dir .bf --unit F1.1 --sub-units "F1.1a: implement — A, F1.1b: review — B" 2>/dev/null)
...
setup_stalled_loop
OUT=$($HARNESS reinit-loop --skip-scope --dir .bf --unit NONEXISTENT --sub-units "X.1: implement — A, X.2: review — B" 2>/dev/null)
...
setup_stalled_loop
OUT=$($HARNESS reinit-loop --skip-scope --dir .bf --unit F1.1 --sub-units "F1.1a: implement — Only one" 2>/dev/null)
...
setup_stalled_loop
OUT=$($HARNESS reinit-loop --skip-scope --dir .bf --unit F1.1 --sub-units "F1.1a: implement — First, F1.1a: review — Duplicate" 2>/dev/null)
...
setup_stalled_loop
OUT=$($HARNESS reinit-loop --skip-scope --dir .bf --unit F1.1 --sub-units "F1.2: implement — Conflicts with existing, F1.1a: review — OK" 2>/dev/null)
...
setup_stalled_loop
$HARNESS reinit-loop --skip-scope --dir .bf --unit F1.1 --sub-units "F1.1a: implement — Part A, F1.1b: review — Part B" >/dev/null 2>/dev/null
HISTORY_LEN=$(python3 -c "import json; d=json.load(open('.bf/loop-state.json')); print(len(d.get('_tick_history',[])))")
...
$HARNESS next-tick --dir .bf >/dev/null 2>/dev/null  # advances to in_progress
...
OUT=$($HARNESS complete-tick --dir .bf --unit F1.1a --status completed --artifacts t.json 2>/dev/null)
...
setup_stalled_loop
$HARNESS reinit-loop --skip-scope --dir .bf --unit F1.1 --sub-units "F1.1a: implement — A, F1.1b: review — B" >/dev/null 2>/dev/null
# After reinit: tick=3 (consumed), new plan has 4 units (F1.1a, F1.1b, F1.2, F1.3)
# Budget = 3 + 4*3 = 15
MAX_TICKS=$(python3 -c "import json; d=json.load(open('.bf/loop-state.json')); print(d.get('_max_total_ticks','MISSING'))")
...
setup_stalled_loop
OLD_HASH=$(python3 -c "import json; d=json.load(open('.bf/loop-state.json')); print(d.get('_plan_hash',''))")
$HARNESS reinit-loop --skip-scope --dir .bf --unit F1.1 --sub-units "F1.1a: implement — A, F1.1b: review — B" >/dev/null 2>/dev/null
NEW_HASH=$(python3 -c "import json; d=json.load(open('.bf/loop-state.json')); print(d.get('_plan_hash',''))")
...
setup_stalled_loop
OUT=$($HARNESS reinit-loop --skip-scope --dir .bf --unit F1.1 --sub-units "this is not a valid format, also bad" 2>/dev/null)
...
setup_stalled_loop
OUT=$($HARNESS reinit-loop --skip-scope --dir .bf --unit F1.1 --sub-units "F1.1a: implement — Build API, F1.1b: review — Check API" 2>/dev/null)
