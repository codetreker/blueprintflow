...
EOF
  echo "# Eval A - review findings" > .bf/nodes/code-review/eval-a.md
  echo "# Eval B - secondary review" > .bf/nodes/code-review/eval-b.md
...
EOF
  echo "# Eval A - test design findings" > .bf/nodes/test-design/eval-a.md
  echo "# Eval B - test design secondary" > .bf/nodes/test-design/eval-b.md
...
EOF
  echo "- fix" > .bf/backlog.md
  $HARNESS transition --from gate --to build --verdict FAIL --flow build-verify --dir .bf 2>/dev/null >/dev/null
}
...
EOF
touch .bf/nodes/build/x
TRANS=$($HARNESS transition --from build --to code-review --verdict PASS --flow build-verify --dir .bf 2>/dev/null || true)
