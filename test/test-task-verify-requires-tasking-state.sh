...
setup() {
  ...
  sed -i.bak 's/^State: Draft/State: Ready/' "$BASE/wo-1/task-a/spec.md" "$BASE/wo-1/task-b/spec.md"
...
grep -qE "^- \[ \] AC-1\|" "$BASE/wo-1/task-a/spec.md" || fail "Ready task AC should not flip before next claims it"
grep -q "^State: Ready" "$BASE/wo-1/task-a/spec.md" || fail "Ready task should stay Ready before next claims it"
cleanup
