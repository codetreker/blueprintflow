...
- [ ] AC-1|verification: 一
- [ ] AC-2|security: 二
- [x] AC-3|verification: 已签到
EOF
)

# flip AC-1
STDOUT=$(node --input-type=module -e "
  import('$REPO_ROOT/bin/lib/write-checkbox.mjs').then(m => {
    process.stdout.write(m.flipCheckbox(process.argv[1], 'AC-1'));
  });
" -- "$INPUT")
assert_match "$STDOUT" "- [x] AC-1|verification" "AC-1 flipped"
...
" -- "$INPUT")
assert_match "$STDOUT" "- [x] AC-3|verification" "AC-3 idempotent"
